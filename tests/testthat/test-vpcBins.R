...
test_that("getBinsSettings returns an error when inapropriate arguments", {
  ...
  expect_error(getBinsSettings(binRange = c(-1, 10)))
  expect_error(getBinsSettings(binRange = c(1, 10, 12)))
  expect_error(getBinsSettings(binRange = c(FALSE)))
  expect_error(getBinsSettings(binRange = c(6.53, 10)))
  expect_error(getBinsSettings(nbBinData = c(-1, 10)))
  expect_error(getBinsSettings(nbBinData = c("a", 10)))
  expect_error(getBinsSettings(nbBinData = c(FALSE)))
  expect_error(getBinsSettings(nbBinData = c(6, 10, 12)))
})

test_that("getBinsSettings returns a binSettingsClass object which is a list", {
  expect_type(getBinsSettings(), "list")
  expect_s3_class(getBinsSettings(), "binSettingsClass")
})
...
test_that("getBinsSettings output take into account argument values", {
  expect_equal(getBinsSettings(is.fixedBins = TRUE, fixedBins = c(5, 6, 10))$is.fixedBins, TRUE)
  expect_equal(getBinsSettings(fixedBins = c(5, 6, 10))$fixedBins, c(5, 6, 10))
  expect_equal(getBinsSettings(fixedBins = c(5, 6, 10))$is.fixedBins, TRUE)
  expect_equal(getBinsSettings(criteria = "equalwidth")$criteria, "equalwidth")
  expect_equal(getBinsSettings(is.fixedNbBins = TRUE)$is.fixedNbBins, TRUE)
  expect_equal(getBinsSettings(nbBins = 15)$nbBins, 15)
  expect_equal(getBinsSettings(nbBins = 15)$is.fixedNbBins, TRUE)
  expect_equal(getBinsSettings(binRange = c(3, 40))$binRange, c(3, 40))
  expect_equal(getBinsSettings(binRange = c(3, 40))$is.fixedNbBins, FALSE)
  expect_equal(getBinsSettings(nbBinData = c(3, 40))$nbBinData, c(3, 40))
  expect_equal(getBinsSettings(nbDataPoints = 80)$nbDataPoints, 80)
})
...
test_that("computeVpcBins returns error when inapropriate arguments", {
  ...
  expect_error(computeVpcBins(data = data, is.binsName = "toto"))
  expect_error(computeVpcBins(data = data, is.binsName = 3))
  expect_error(computeVpcBins(data = data, dataName = 3))
  expect_error(computeVpcBins(data = data, dataName = FALSE))
})
...
test_that("computeVpcBins add binsName column when is.binsName = TRUE", {
  data <- c(1, 2, 3, 1, 1, 2, 5, 5, 3)
  mockery::stub(
    computeVpcBins,
    ".getBins",
    data.frame(bins = c(1, 2, 3), bins_start = c(1, 3, 6),
               bins_stop = c(1, 4, 7), bins_middles = c(1, 3.5, 6.5))
  )
  res1 = computeVpcBins(data, is.binsName = TRUE)
  res2 = computeVpcBins(data, is.binsName = TRUE, dataName = "toto")
  expect_equal(res1$binsName, c("P(obs=1)", "P(3<=obs<=4)", "P(6<=obs<=7)"))
  expect_equal(res2$binsName, c("P(toto=1)", "P(3<=toto<=4)", "P(6<=toto<=7)"))
})
...
test_that(".getBins create categories from data when categorical binning with no categories specified", {
  ...
  expectedBins <- data.frame(
    ...
    bins_middles = categories, bins = seq_len(length(categories))
  )
...
test_that(".getBins returns fixed bins when used fixed bins", {
  ...
  expectedBins <- data.frame(
    ...
    bins_middles = c(mean(c(1, 1, 1, 2, 2)), mean(c(3, 5, 5, 3)), mean(c(6))), bins = seq_len(3)
  )
...
test_that(".getBins returns result of computeBins function when no fixed bins", {
  ...
  expectedBins <- data.frame(
    ...
    bins_middles = c(10, 74), bins = seq_len(2)
  )
  expect_equal(.getBins(data, settings = settings), expectedBins)
})
